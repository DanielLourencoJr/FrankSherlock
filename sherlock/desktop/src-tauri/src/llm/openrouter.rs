...
pub fn openrouter_generate(
    ...
    if let Some(img_path) = image_path {
        let bytes = match std::fs::read(img_path) {
            Ok(b) => b,
            Err(e) => return OpenRouterResponse::error(format!("image_read_error: {e}")),
        };

        if bytes.len() as u64 > MAX_IMAGE_BYTES {
            return OpenRouterResponse::error(format!(
                "image_too_large: {} bytes exceeds 10 MB limit",
                bytes.len()
            ));
        }
    ...
    for attempt in 0..=MAX_RATE_LIMIT_RETRIES {
        let mut resp = match agent
            ...
            .header("HTTP-Referer", "https://franksherlock.app")
            .header("X-Title", "Frank Sherlock")
            .send_json(&payload)
        {
            Ok(r) => r,
            Err(e) => {
        ...
        if status == 429 {
            ...
            let wait_secs: u64 = retry_after.parse().unwrap_or(10);

            log::warn!(
                "OpenRouter rate-limited (attempt {}/{}): retrying after {}s",
                attempt + 1,
            ...
            return OpenRouterResponse::error(
                "payload_too_large: image + request exceeds size limit".to_string(),
            );
        } else {
            let msg = if let Ok(parsed) = serde_json::from_str::<Value>(&body) {
                let error_msg = parsed
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown");
                format!("openrouter_api_error: {error_msg}")
            } else {
                format!("http_error: {status} — {body}")
            };
            return OpenRouterResponse::error(msg);
        }
    }

    OpenRouterResponse::error(format!("rate_limit_exhausted: {last_error}"))
}

/// Whether the API key appears to be configured.
/// OpenRouter keys start with `sk-or-` and are at least 10 chars.
...
mod tests {
    ...
    fn api_key_configured_accepts_valid_key() {
        assert!(is_api_key_configured("sk-or-v1-abcdef1234567890abcdef"));
    }
